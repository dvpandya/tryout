...
print("No. of moles of NH4OH decomposed in first tank = ",X1*Cin*F1)

print("No. of moles of NH4OH decomposed in second tank = ",X2*CA2[100]*F1)

print("No. of moles of NH4OH decomposed in third tank = ",X3*CA3[100]*F1)

conv_1=X1*Cin*F1  # Number of moles of nh4oh converted in tank 1

conv_2=X2*CA2[100]*F1   # Number of moles of nh4oh converted in tank 2

conv_3=X3*CA3[100]*F1   # Number of moles of nh4oh converted in tank 3
'''
Enthalpy Balance'''

H_in_1=Cin*F1*H_nh4oh
H_out_1=(Cin*F1-conv_1)*H_nh4oh+conv_1*H_ammonia+conv_1*H_water
...
def enthalpy(T):
    f=Delta_H_1+R*((Cin*F1-conv_1)*(A_nh4oh*(T-298.15)+B_nh4oh*(T**2-298.15**2)/2+C_nh4oh*(T**3-298.15**3)/3)+ (conv_1)*(A_water*(T-298.15)+B_water*(T**2-298.15**2)/2+C_water*(T**3-298.15**3)/3)+(conv_1)*(A_amm*(T-298.15)+B_amm*(T**2-298.15**2)/2-D_amm*(T**-1-298.15**-1)/2))
    return f

T_soln=fsolve(enthalpy,300)

print("The temp. in the first tank is" ,T_soln)
...
for i in range(1,100):
    ...
    Temp_soln[i]=fsolve(graph,Temp_soln[i-1])
